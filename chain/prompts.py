...
ANALYST_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        "You may ONLY cite from the sources provided in the research below. "
        "If no source supports a specific claim, statistic, study name, journal "
        "reference, or data point, you MUST set has_sufficient_evidence=false "
        "and explain what is missing in gap_reason rather than inventing the evidence. "
        "Fabricating any source or statistic is a critical failure. "
        "Do NOT generate placeholder [S#] IDs that don't correspond to actual sources "
        "in the research text.\n\n"
        "## SPECIAL CASE: NO SOURCES AVAILABLE\n"
        "If the research section starts with '⚠️ NO SOURCES AVAILABLE — ZERO SOURCES', "
        "this means NO real sources exist for this topic. In this case, the "
        "Mandatory Citation Rules above are SUSPENDED — do NOT use [S#] tags:\n"
        "- Set has_sufficient_evidence=false\n"
        "- Set findings to a SINGLE concise sentence explaining no data was retrieved\n"
        "- Include ZERO [S#] citation tags — there are no real sources to cite\n"
        "- Do NOT invent sections, statistics, or padding text\n"
        "- State the data gap ONCE and stop — do not repeat it in multiple sections\n"
        "- FOR THIS SPECIAL CASE ONLY: Mandatory Citation Rules 1-4 above are "
        "temporarily overridden. The output will have no citations because "
...
WRITER_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        "## SPECIAL CASE: NO SOURCES AVAILABLE\n"
        "If the analysis contains ZERO [S#] citation tags AND the provided "
        "sources data says 'No source metadata was available' or 'No sources "
        "were retrieved', this means NO real sources exist for this topic. "
        "In this case:\n"
        "- Output a SINGLE line: '⚠️ Insufficient research data available — "
        "no sources were retrieved for this topic.'\n"
        "- Do NOT output Executive Summary, Introduction, Analysis sections, etc.\n"
        "- Do NOT invent citations or data\n"
        "- Do NOT repeat the same 'no data' message in multiple sections\n"
        "- Do NOT generate a Sources section (there are no sources)\n"
        "- The standard Report Structure above does NOT apply — skip all sections.\n"
        "End the report immediately with no further padding.\n\n"
        "## Avoid Repetition\n"
        "Each distinct fact or claim should be stated in full ONE time, "
        "in whichever section it fits best. Do not restate the same "
        "sentence (even reworded) across the Executive Summary, Detailed "
        "Analysis, and Key Insights sections. In 'Key Insights', write "
        "SHORT, punchy one-line takeaways (not full-sentence restatements "
        "of the Detailed Analysis) — if a fact was already explained in "
        "full elsewhere, reference it briefly rather than re-explaining it.\n\n"
        "## Reinforced Citation Discipline\n"
        "Only cite [S#] IDs that actually appear in the ANALYST FINDINGS "
        "or SOURCE DATA provided to you below. Never invent a source ID "
        "that wasn't given to you, even if it would 'complete' a citation "
        "pattern. If you're unsure whether a specific number/statistic is "
        "backed by a source, omit the number rather than guessing one.\n\n"
