...
class ResearchQuestion(BaseModel):
    ...
    - Specific and checkable (answerable with a concrete fact, event, name, or number)
    - Time-anchored (if a year is mentioned in the topic, include it)
    - Paired with an optimized search query (not just the full sentence)
    """

    question: str = Field(description="The specific sub-question to research — must be answerable with a concrete fact, event, name, or number")
    search_query: str = Field(
        description="Optimized search query for this sub-question (short, keyword-focused, not a full sentence)"
    )
...
# ── Source Models (tracked through the entire pipeline) ────────────────────

class Source(BaseModel):
    """A single search result with an ID-tag that survives to the final report.

    Each Source is assigned a unique ID (S1, S2, ...) when search results are
    processed. These IDs are used as inline citation tags like [S1] in the
    analyst findings and final report, so every claim is traceable.
    """

    id: str = Field(description="Unique source ID like S1, S2, ...")
    url: str = Field(description="URL of the source")
    title: str = Field(description="Title of the source page")
    snippet: str = Field(description="The actual retrieved text / body snippet")


class SubQuestionResult(BaseModel):
    """Result from researching a single sub-question.

    Contains the structured sources found AND the synthesized findings text
    with inline [S#] citation tags referencing those sources.
    """

    question: str = Field(description="The sub-question that was researched")
    sources: list[Source] = Field(description="Sources found for this sub-question")
    raw_findings: str = Field(
        description="Synthesized findings text with inline [S#] citation tags"
    )
...
class AnalysisFindings(BaseModel):
    """Structured analyst output with citation-forced findings.

    Every factual claim in `findings` must include an inline [S#] citation
    tag referencing a specific source. If sources don't answer the question,
    the analyst sets has_sufficient_evidence=False and explains why in
    gap_reason — never disguises gaps as hedging prose in the findings.
    """

    findings: str = Field(
        description=(
            "Synthesized findings with inline [S#] citation tags. "
            "Every factual claim must cite its source. Never write hedges "
            "or vague statements."
        )
    )
    has_sufficient_evidence: bool = Field(
        description=(
            "True if the research sources actually answer the sub-questions. "
            "False if sources are insufficient — in that case, explain in gap_reason."
        )
    )
    gap_reason: Optional[str] = Field(
        default=None,
        description=(
            "If has_sufficient_evidence=False, explain what information is "
            "missing and what would be needed. Never disguise this as hedge prose."
        ),
    )
